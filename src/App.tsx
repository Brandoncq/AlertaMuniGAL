...
const getAlertIcon = (estado: string, isDragging: boolean = false) => {
  if (isDragging)
    return getDivIcon("#f97316", alertSvg, false, "border-dashed"); // Orange dragging

  switch (estado) {
    case "PENDIENTE":
      return getDivIcon("#ef4444", alertSvg, true); // red-500
    case "ASIGNADO":
      return getDivIcon("#f97316", alertSvg, false); // orange-500
    case "DESPLIEGUE":
      return getDivIcon("#a855f7", alertSvg, false); // purple-500
    case "INTERVENCION":
      return getDivIcon("#3b82f6", alertSvg, true); // blue-500
    case "ATENDIDO":
      return getDivIcon("#22c55e", checkSvg, false); // green-500
    default:
      return getDivIcon("#94a3b8", alertSvg, false); // slate-400
  }
};

const getSerenoIcon = (estado: string, tipo: string) => {
...
const OPERADORES_MOCK = [
  {
    ...
    nombres: "Luis",
    apellidos: "Paredes",
    correo: "operador1@hackathon.pe",
  },
  {
    id: 3,
    nombres: "Carla",
    apellidos: "Díaz",
    correo: "operador2@hackathon.pe",
  },
];
...
export default function App() {
  ...
  const [selectedAlertaDetail, setSelectedAlertaDetail] = useState<any | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const unsubDash = subscribeToDashboard((event, data) => {
      ...
      if (event === "ALERTA_CREADA") {
          ...
          [
            {
              id: Date.now(),
              title: "Nueva Alerta",
              message: `Alerta #${data.id} reportada.`,
              time: new Date(),
            },
            ...prev,
          ].slice(0, 50),
        ...
        setAlertas((prev) =>
          prev.map((a) =>
            a.id === data.alerta_id
              ? { ...a, ubicacion_incidencia: data.ubicacion_incidencia }
              : a,
          ),
        );
        if (selectedAlerta?.id === data.alerta_id) {
          setSelectedAlerta((prev: any) => ({
            ...prev,
        ...
        if (
          ["ATENDIDO", "RECHAZADO", "RECHAZADO_MANUAL"].includes(nuevoEstado)
        ) {
          ...
          setSelectedAlerta((prev: any) =>
            prev?.id === data.alerta_id ? null : prev,
          );
          ...
          setAlertas((prev) =>
            prev.map((a) =>
              a.id === data.alerta_id
                ? { ...a, estado_actual: nuevoEstado }
                : a,
            ),
          );
          setSelectedAlerta((prev: any) =>
            prev?.id === data.alerta_id
              ? { ...prev, estado_actual: nuevoEstado }
              : prev,
          );
        }
        setNotifications((prev) =>
          [
            {
              ...
              message: `Alerta #${data.alerta_id || data.id} pasó a ${nuevoEstado}`,
              time: new Date(),
            },
            ...prev,
          ].slice(0, 50),
    ...
    const unsubGlobal = subscribeToGlobalEvents((event, data) => {
      if (event === "SERENO_ACEPTO") {
          ...
          [
            {
              ...
              message: `Patrulla en camino para alerta #${data.alerta_id}.`,
              time: new Date(),
            },
            ...prev,
          ].slice(0, 50),
        ...
        loadAllSerenos();
        alert(
          `¡Patrulla en camino! El sereno asignado a la alerta #${data.alerta_id} ha aceptado la misión y está en despliegue.`,
        );
      } else if (
        event === "SERENO_RECHAZO_TIMEOUT" ||
        event === "SERENO_RECHAZO_MANUAL"
      ) {
        setNotifications((prev) =>
          [
            {
              ...
              message: `El sereno rechazó la alerta #${data.alerta_id}.`,
              time: new Date(),
            },
            ...prev,
          ].slice(0, 50),
        ...
        loadAllSerenos();
        alert(
          `La asignación para la alerta #${data.alerta_id} fue rechazada o expiró el tiempo.`,
        );
      }
    });
  ...
  const handleSelectAlerta = async (alerta: any) => {
    ...
    if (alerta.estado_actual === "PENDIENTE") {
      calculateSerenos(
        alerta.ubicacion_incidencia.lat,
        alerta.ubicacion_incidencia.lng,
      );
    }
  };
  ...
  const calculateSerenos = async (lat: number, lng: number) => {
    ...
    try {
      ...
      if (serenosCercanos.length > 0) {
        const coordsString = serenosCercanos
          .map((s: any) => `${s.ubicacion_actual.lng},${s.ubicacion_actual.lat}`)
          .join(";");
        ...
        try {
          ...
          if (osrmData.code === "Ok" && osrmData.routes) {
            ...
            const updatedSerenos = serenosCercanos.map((s: any, index: number) => {
            ...
            updatedSerenos.sort(
              (a: any, b: any) =>
                a.tiempo_estimado_llegada_min - b.tiempo_estimado_llegada_min,
            );
  ...
  const handleUpdateLocation = async () => {
    ...
    try {
      await actualizarUbicacionAlerta(
        selectedAlerta.id,
        draggedLocation.lat,
        draggedLocation.lng,
      );
      const updatedAlerta = {
        ...selectedAlerta,
        ubicacion_incidencia: draggedLocation,
      };

      setAlertas((prev) =>
        prev.map((a) => (a.id === updatedAlerta.id ? updatedAlerta : a)),
      );
  ...
  const mapCenter: [number, number] = draggedLocation
    ? [draggedLocation.lat, draggedLocation.lng]
    : selectedAlerta
      ? [
          selectedAlerta.ubicacion_incidencia.lat,
          selectedAlerta.ubicacion_incidencia.lng,
        ]
      : defaultCenter;
            ...
            <div className="bg-red-50 p-6 border-b border-red-100 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-red-900">
                  Rechazar Alerta #{selectedAlerta.id}
                </h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                >
                  ...
                  <option value="Datos Insuficientes">
                    Datos Insuficientes
                  </option>
                  <option value="Fuera de Jurisdicción">
          ...
          <div className="flex items-center justify-between bg-slate-800 p-2 rounded-lg text-sm relative">
            ...
            <div className="flex items-center gap-3">
              ...
              {selectedAlerta && (
                <button
                  ...
                  className="text-xs flex items-center bg-slate-700 px-2 py-1 rounded hover:bg-slate-600 transition-colors"
                >
            ...
            {showNotifications && (
              <div className="absolute top-12 right-0 w-80 bg-white shadow-2xl rounded-xl z-50 border border-slate-200 overflow-hidden flex flex-col max-h-96">
                <div className="bg-slate-100 p-3 border-b text-slate-700 font-bold text-sm">
                ...
                <div className="flex-1 overflow-y-auto p-2">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <div
                        key={n.id}
                        className="p-2 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors"
                      >
        ...
        {selectedAlerta ? (
          <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
            <div className="flex justify-between items-start mb-6 border-b pb-4">
              <div>
                <h2 className="text-2xl font-black text-slate-800">
                  Alerta #{selectedAlerta.id}
                </h2>
                <span
                  className={`inline-block mt-2 text-xs font-bold px-3 py-1 rounded-full text-white ${estadoColors[selectedAlerta.estado_actual] || "bg-gray-500"}`}
                >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                  <h3 className="font-bold text-slate-700 text-sm mb-3 flex items-center gap-2">
                    <User size={16} className="text-blue-500" /> Datos del
                    Solicitante
                  </h3>
                  <p className="text-lg font-bold text-slate-900">
                    {selectedAlertaDetail.ciudadano_contacto.nombre_completo}
                  </p>
                  <p className="text-slate-600 flex items-center gap-2 mt-1">
                    <Phone size={14} />{" "}
                    {selectedAlertaDetail.ciudadano_contacto.celular ||
                      "No registrado"}
                  </p>
                </div>

                {/* Contactos Familiares */}
                {selectedAlertaDetail.ciudadano_contacto.contactos_referencia
                  ?.length > 0 && (
                  <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                    <h3 className="font-bold text-slate-700 text-sm mb-3 flex items-center gap-2">
                      <Users size={16} className="text-orange-500" /> Contactos
                      de Emergencia
                    </h3>
                    <div className="space-y-2">
                      {selectedAlertaDetail.ciudadano_contacto.contactos_referencia.map(
                        (c: any, i: number) => (
                          <div
                            key={i}
                            className="flex justify-between text-sm bg-slate-50 p-2 rounded-lg"
                          >
                            <span className="font-semibold text-slate-800">
                              {c.nombre_referencia} ({c.tipo_relacion})
                            </span>
                ...
                <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                  <h3 className="font-bold text-slate-700 text-sm mb-3 flex items-center gap-2">
                    <AlertCircle size={16} className="text-red-500" /> Detalle
                    de Incidencia
                  </h3>
                ...
                {selectedAlertaDetail.sereno_asignado && (
                  <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                    <h3 className="font-bold text-slate-700 text-sm mb-3 flex items-center gap-2">
                      <Shield size={16} className="text-indigo-500" /> Unidad
                      Asignada
                    </h3>
                    <div className="bg-indigo-50 p-3 rounded-xl border border-indigo-100">
                      <div className="flex justify-between items-start mb-1">
                        <p className="font-bold text-slate-900">
                          {selectedAlertaDetail.sereno_asignado.nombre_completo}
                        </p>
                        <span className="text-[10px] font-black text-indigo-600 bg-indigo-100 px-1.5 py-0.5 rounded uppercase">
                          {selectedAlertaDetail.sereno_asignado.tipo_unidad ||
                            "UNIDAD"}
                        </span>
                      </div>
                      <p className="text-slate-600 text-sm flex items-center gap-2 mb-2">
                        <Phone size={14} />{" "}
                        {selectedAlertaDetail.sereno_asignado.celular ||
                          "No registrado"}
                      </p>
                      <div className="flex justify-between text-xs font-bold border-t border-indigo-100 pt-2 mt-1">
                        ...
                        <span className="text-slate-500">
                          ETA orig.:{" "}
                          <span className="text-indigo-600">
                            {selectedAlertaDetail.sereno_asignado.eta} min
                          </span>
                        </span>
                      </div>
                    </div>
                ...
                {selectedAlertaDetail.formulario_ciudadano && (
                  <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                    <h3 className="font-bold text-slate-700 text-sm mb-3 flex items-center gap-2">
                      ...
                      {selectedAlertaDetail.formulario_ciudadano.titulo ||
                        "Informe del Sereno"}
                    </h3>
                    <div className="space-y-3">
                      {Object.entries(
                        selectedAlertaDetail.formulario_ciudadano.respuestas ||
                          {},
                      ).map(([key, val]) => (
                        <div
                          key={key}
            ...
            {selectedAlerta.estado_actual === "PENDIENTE" && (
              <div className="mt-6 bg-blue-50 border border-blue-200 p-4 rounded-xl text-blue-800 text-sm">
                ...
                <p>
                  Puedes <strong>arrastrar el marcador rojo en el mapa</strong>{" "}
                  para afinar el "Lugar de los Hechos" real tras confirmar por
                  teléfono.
                </p>
              </div>
            )}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 flex flex-col">
            ...
            <div className="flex justify-between items-center mb-2 px-1">
              ...
              <span
                className={`${activeTab === "activas" ? "bg-blue-100 text-blue-800" : "bg-slate-200 text-slate-700"} font-bold px-2 py-1 rounded-md text-xs`}
              >
                {activeTab === "activas"
                  ? alertas.length
                  : todasAlertas.filter(
                ...
                alertas.map((a) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <span
                        className={`text-xs font-bold px-3 py-1 rounded-full text-white ${estadoColors[a.estado_actual] || "bg-gray-500"}`}
                      >
                        {a.estado_actual}
                      </span>
                ...
                .map((a) => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded text-white ${estadoColors[a.estado_actual] || "bg-gray-500"}`}
                      >
                        {a.estado_actual}
                      </span>
      ...
      <div className="flex-1 relative flex flex-col">
        ...
        {draggedLocation && (
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-[1000] bg-white p-3 rounded-full shadow-2xl flex items-center gap-4 border-2 border-orange-400 animate-bounce">
            <span className="font-bold text-slate-800 text-sm">
              ¿Confirmar nuevo Lugar de los Hechos?
            </span>
            <button
        ...
        <div className="flex-1 relative">
          ...
          >
            ...
            {allSerenos.map((s) => (
              <Marker
                key={`global-${s.id_usuario}`}
                position={[s.ubicacion_actual.lat, s.ubicacion_actual.lng]}
                icon={getSerenoIcon(s.estado_disponibilidad, s.tipo)}
              >
                <Popup>
            ...
            <MarkerClusterGroup chunkedLoading>
              {alertas.map((a) => {
                ...
                const position = isDragging
                  ? ([draggedLocation.lat, draggedLocation.lng] as [
                      number,
                      number,
                    ])
                  : ([
                      a.ubicacion_incidencia.lat,
                      a.ubicacion_incidencia.lng,
                    ] as [number, number]);
            ...
            {selectedAlerta?.estado_actual === "PENDIENTE" &&
              !draggedLocation &&
              serenosParaAsignar.map((s) => (
                <Marker
                  key={`asignar-${s.id_usuario}`}
                  position={[s.ubicacion_actual.lat, s.ubicacion_actual.lng]}
                  icon={getSerenoIcon("OCUPADO", s.tipo)}
                >
                  <Popup>
                    <strong>
                      {s.nombres} {s.apellidos}
                    ...
                    <br />
                    Llega en: {s.tiempo_estimado_llegada_min} min
                    <br />A {s.distancia_estimada_mts} mts
                  </Popup>
                </Marker>
              ))}
          </MapContainer>
              ...
              {loadingSerenos ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[30vh] overflow-y-auto pr-1">
                  {serenosParaAsignar.map((s, index) => (
                    <div
                      key={s.id_usuario}
                      className="bg-white border border-slate-200 p-3 rounded-xl hover:border-blue-400 hover:shadow-md transition-all"
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded w-max mb-1 uppercase">
                            {s.tipo || "UNIDAD"}
                          </span>
                      ...
                      <div className="flex gap-3 mb-3">
                        <div className="flex items-center gap-1 text-slate-700">
                          ...
                          <span className="font-bold text-sm">
                            {s.tiempo_estimado_llegada_min}m
                          </span>
                        </div>
